...
export default function Navbar() {
  const { activeTab, setActiveTab, isAuthenticated, setIsAuthenticated } = useAppContext();
        ...
        <div className="space-x-2">
          ...
          {isAuthenticated ? (
            <>
              ...
              <Link to='/' onClick={handleClickLogout} className={`inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-primary  hover:text-white h-10 px-4 py-2 bg-white hover:bg-primary text-primary`}>
                Logout
              </Link>
            </>
          ) :
